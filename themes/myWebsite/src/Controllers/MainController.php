...
class MainController extends Controller
{
    ...
    public function setCookieTheme(Request $request)
    {
        ...
        if ($request->theme=="style-dark"){
            if (option('info_logo_dark')){
                $logo=asset(option('info_logo_dark', theme_asset('img/logo.png')));
            }else{
                $logo=asset(option('info_logo', theme_asset('img/logo.png')));
            }
        }else{
            $logo=asset(option('info_logo', theme_asset('img/logo.png')));
        }
    ...
    {
        Cookie::forget('setCookieColor');
        Cookie::queue('setCookieColor',  $request->theme,36000*24);
    }
}
