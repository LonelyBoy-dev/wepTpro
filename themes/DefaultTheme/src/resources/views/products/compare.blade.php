...
@section('content')
              ...
              <table class="table">
                  ...
                  <tr>
                    ...
                    @foreach ($products as $product)
                      <td>
                        <div class="comparison-item">
                          @if ($products->count() > 1)
                            <a href="{{ remove_id_from_url($product->id) }}"><span class="remove-item"><i class="mdi mdi-close"></i></span></a>
                          @endif
                ...
                <tbody>
                  ...
                  @foreach ($groups as $group)
                    ...
                    @php
                        $specifications = \DB::table('product_specification')->where('specification_group_id', $group->id)->whereIn('product_id', $products->pluck('id'))->orderBy('group_ordering')->get();
                    @endphp
                    ...
                    @foreach ($specifications->unique('specification_id') as $specification)
                      ...
                      <tr>
                        ...
                        @foreach ($products as $product)
                          @php
                            $specification_value = $specifications->where('specification_id', $spec->id)->where('product_id', $product->id)->first();
                          @endphp
                  ...
                  <tr>
                    <th></th>
                    @foreach ($products as $product)
                      <td><a class="btn btn-primary btn-sm btn-block" href="{{ route('front.products.show', ['product' => $product]) }}">مشاهده و خرید</a></td>
                    @endforeach
  ...
  @if ($products->count() < 3)
    ...
    @php
      $similar_products = \App\Models\Product::whereNotIn('id', $products->pluck('id'))->where('spec_type_id', $products->first()->spec_type_id)->orderByStock()->take(12)->get();
    @endphp
    @include('front::products.partials.compare-modal', ['similar_products' => $similar_products, 'current_url' => url()->current()])
  @endif
