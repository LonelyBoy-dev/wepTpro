        ...
        <form id="products-filter-form" action="{{ route('front.products.category-products', ['category' => $category]) }}">
            ...
            <div class="col-12 filter-product mb-3">
                <div class="accordion" id="accordionExample">
                    ...
                    @php
                        $products_id = get_category_products_id($category);
                    @endphp
                    ...
                    @foreach ($category->getFilter()->related()->orderBy('ordering')->get() as $filter)
                        ...
                        @switch($filter->filterable_type)
                            @case('App\Models\Specification')
                                ...
                                @php
                                    $spec_values = \DB::table('product_specification')->whereIn('product_id', $products_id)->where('specification_id', $filter->filterable->id)->get()->unique('value')->pluck('value')->toArray();
                            ...
                            @case('App\Models\AttributeGroup')
                                ...
                                @php
                                    ...
                                    $group_values = \App\Models\Attribute::where('attribute_group_id', $filter->filterable->id)->whereIn('id', $attributes)->get();
                                @endphp
