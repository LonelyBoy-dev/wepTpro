...
class AddPost_LearnsourceSite_PhpCat extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'post:LearnsourceSite_PhpCat';
    ...
    {
        $client = new Client();
        $crawler = $client->request('GET', 'https://learnsource.net/article/List?order=latest&categoryName=%D9%BE%DB%8C+%D8%A7%DA%86+%D9%BE%DB%8C');
        $crawler->filter('.container.m-auto.px-4.mt-8 .grid.grid-cols-12.gap-6')->each(function ($node) {
            ...
            if (empty($post)) {
                ...
                $inside_post->each(function ($item) {
                    ...
                    $category_id=Category::where('slug',"پ-اچ-پی")->first();
                    $category=PostCategory::where(['post_id'=>$post->id,'category_id'=>$category_id->id])->first();
